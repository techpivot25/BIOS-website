...
import { PUBLIC_CLOUD_SUB_OFFERINGS, PublicCloudSubOffering } from '../data/publicCloudData';

interface PublicCloudPageProps {
  onNavigate: (page: string, subId?: string) => void;
  onOpenContact: (subject?: string) => void;
  initialSubOfferingId?: string;
}

export const PublicCloudPage: React.FC<PublicCloudPageProps> = ({
  onNavigate,
  onOpenContact,
  initialSubOfferingId,
}) => {
  const [selectedSubId, setSelectedSubId] = useState<string>(
    initialSubOfferingId || 'multi-cloud'
  );
  ...
  const activeOffering = PUBLIC_CLOUD_SUB_OFFERINGS.find(
    (item) => item.id === selectedSubId
  ) || PUBLIC_CLOUD_SUB_OFFERINGS[0];

  const handleSelectSub = (id: string) => {
    setSelectedSubId(id);
    window.location.hash = `public-cloud/${id}`;
  };
  ...
  const getSubIcon = (id: string, className = "w-5 h-5") => {
    switch (id) {
      case 'multi-cloud': return <Layers className={className} />;
      ...
      <div className="bg-gradient-to-b from-slate-900 via-slate-950 to-slate-950 border-b border-slate-800/80 pt-10 pb-8 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex items-center gap-2 text-xs font-semibold text-slate-400 mb-4">
            ...
            <span className="text-slate-300">Public Cloud</span>
            <ChevronRight className="w-3.5 h-3.5 text-slate-600" />
            <span className="text-blue-400 font-bold">{activeOffering.name}</span>
          </div>
          ...
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
            <div>
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-bold bg-blue-500/10 border border-blue-500/20 text-blue-400 mb-3">
                ...
                <span>HYPERSCALE ORCHESTRATION & GCC REGIONAL HUBS</span>
              </div>
              <h1 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold text-white tracking-tight leading-tight">
                Managed Public & Multi-Cloud Solutions
              </h1>
              <p className="mt-2 text-base sm:text-lg text-slate-300 max-w-3xl leading-relaxed">
                Single-pane multi-cloud governance, certified Microsoft Azure in UAE regions, and dedicated ExpressRoute private fiber connectivity.
              </p>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3 shrink-0">
              <button
                ...
                className="px-4 py-2.5 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white font-bold text-xs shadow-lg shadow-blue-950/40 hover:scale-105 transition-all flex items-center gap-2 cursor-pointer"
              >
            ...
            {PUBLIC_CLOUD_SUB_OFFERINGS.map((sub) => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-xs font-bold whitespace-nowrap transition-all duration-200 cursor-pointer ${
                    isSelected
        ...
        <div className="max-w-7xl mx-auto space-y-12">
            ...
            <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
              <div className="lg:col-span-7 space-y-5">
                ...
                <div className="flex flex-wrap items-center gap-2.5">
                  <span className="px-3 py-1 rounded-full text-xs font-extrabold uppercase tracking-wider bg-blue-500/20 text-blue-400 border border-blue-500/30">
                ...
                <div className="pt-2 flex flex-wrap items-center gap-3">
                  <button
                    onClick={() => onOpenContact(`Inquiry: Public Cloud - ${activeOffering.name}`)}
                  ...
                  >
                    ...
                    <span>Chat with Cloud Architect</span>
                  </button>
              ...
              <div className="lg:col-span-5 space-y-4">
                <div className="rounded-2xl bg-slate-950/80 border border-slate-800 p-5 sm:p-6 space-y-4 shadow-xl">
                  <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                    ...
                    <span className="text-[10px] text-emerald-400 font-mono font-bold">Certified Partner</span>
                  </div>
                  ...
                  {/* Compliance List */}
                  <div className="pt-2">
                    <p className="text-[11px] font-semibold text-slate-400 mb-2">Frameworks & Accreditations:</p>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                <span className="text-xs font-extrabold text-indigo-400 uppercase tracking-wider">
                  MANAGED HYPERSCALE CAPABILITIES
                </span>
                <h3 className="text-xl sm:text-2xl font-bold text-white">
                  Operational Control & Engineering Management
                </h3>
              </div>
              <button
                onClick={() => onOpenContact(`FinOps & Architecture Review for ${activeOffering.name}`)}
                className="px-4 py-2 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 text-xs font-semibold self-start transition-colors cursor-pointer"
              >
                Request FinOps Review
              </button>
            </div>
          ...
          <div className="space-y-4">
            <div>
              ...
              <h3 className="text-2xl font-bold text-white tracking-tight">
                Recommended Production Workloads for {activeOffering.name}
              </h3>
            </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                <span className="text-xs font-extrabold text-blue-400 uppercase tracking-wider">
                  HYPERSCALE & GOVERNANCE ECOSYSTEM
                </span>
                <h3 className="text-xl sm:text-2xl font-bold text-white">
                  Cross-Comparison: Public & Multi-Cloud Offerings
                </h3>
                <p className="text-xs text-slate-400 mt-1">
                  Evaluate governance scopes, regional interconnects, and operational capabilities.
                </p>
              </div>
            </div>
              ...
              <table className="w-full text-left text-xs text-slate-300">
                <thead className="bg-slate-950 text-slate-400 font-bold uppercase tracking-wider border-b border-slate-800">
                  <tr>
                    ...
                    <th className="py-3 px-4">Core Model</th>
          ...
          {/* BOTTOM CONSULTATION & FINOPS BANNER */}
          <div className="rounded-3xl bg-gradient-to-r from-blue-900/40 via-slate-900 to-indigo-900/30 border border-slate-800 p-8 sm:p-10 flex flex-col md:flex-row items-center justify-between gap-6">
            <div className="space-y-2 text-center md:text-left">
              <span className="text-xs font-extrabold text-orange-400 uppercase tracking-wider">
                LOOKING TO OPTIMIZE OR MIGRATE TO HYPERSCALE?
              </span>
              <h3 className="text-2xl sm:text-3xl font-extrabold text-white">
                Book a Free Multi-Cloud & FinOps Assessment
              </h3>
              <p className="text-sm text-slate-300 max-w-2xl">
                Our certified Microsoft and AWS architects analyze your subscription spend, network topology, and security benchmarks to unlock up to 34% in cost savings.
              </p>
            </div>

            <div className="flex flex-col sm:flex-row items-center gap-3 shrink-0">
              <button
                onClick={() => onOpenContact(`FinOps Assessment: ${activeOffering.name}`)}
